...
static inline void portUpdateScreen(uint8_t* buf){
    ...
    sDRV_ST7305_UpdateScreen(buf,(SDRV_ST7305_H / 4 * SDRV_ST7305_W / 2));
}
...
void sG2D::setDot(uint16_t old_x,uint16_t old_y,bool dot_en){
    if((old_x >= scr_w) || (old_y >= scr_h)) return;

    //逆时针旋转90度矩阵变换
    uint16_t x = scr_h - 1 - old_y;
    uint16_t y = old_x;
    ...
    uint32_t write_byte_index = real_y*42+real_x;
    uint32_t one_two = (y % 2 == 0)?0:1; // 0 1
    uint32_t line_bit_4 = x % 4; // 0 1 2 3
    uint8_t write_bit = 7-(line_bit_4*2+one_two);


    if(dot_en){
        draw_buf[write_byte_index] |=  (1 << write_bit);
    }else{
        draw_buf[write_byte_index] &= ~(1 << write_bit);
    }
}
...
bool sG2D::getDot(uint16_t x, uint16_t y){
    ...
    uint32_t real_x = x/4; // 0->0, 3->0, 4->1, 7->1
    uint32_t real_y = y/2; // 0->0, 1->0, 2->1, 3->1
    uint32_t write_byte_index = real_y*SDRV_ST7305_W+real_x;
    uint32_t one_two = (y % 2 == 0)?0:1; // 0 1
    uint32_t line_bit_4 = x % 4; // 0 1 2 3
    uint8_t write_bit = 7-(line_bit_4*2+one_two);

    // return (draw_buf[x + scr_w * (y / 8)]& (1 << (y % 8))) != 0;
    return draw_buf[write_byte_index] ? 1 : 0;
...
void sG2D::write_string(uint16_t old_x,uint16_t old_y,const char* str){
    //逆时针旋转90度矩阵变换
    // uint16_t x = scr_h - 1 - old_y;
    // uint16_t y = old_x;
    uint16_t x = old_x;
